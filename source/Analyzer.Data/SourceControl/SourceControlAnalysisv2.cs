...
{
    public class SourceControlAnalysisv2 : ISourceControlAnalysis
    {
        ...
        // todo : this needs finer grained objects instead of raw aggregation 
        private List<DeveloperStats> Build_Stats_For_Range(IList<Author> authors, DateTime start, DateTime end)
        {
            ...
            {
                var commitStats = Commit_Stats(developer, start, end);
                var stats = new DeveloperStats
                {
                    ...
                    //LinesOfChangePerHour = commitStats.ChangePerHour,
                    //LinesAdded = commitStats.Added,
                    //LinesRemoved = commitStats.Removed,
                    //Churn = commitStats.Churn,
                    //Rtt100 = commitStats.Rtt100,
                    //Ptt100 = commitStats.Ptt100
                };
        ...
        private List<CommitStat> Commit_Stats(Author developer, DateTime start, DateTime end)
        {
            var stats = new List<CommitStat>();
            ...
            foreach (var commit in developerCommits)
            {
                ...
                {
                    stats.Add(BuildFirstCommitStats(commit));
                    continue;
                }

                stats.AddRange(BuildCommitStats(commit)); 
            }
        ...
        private List<CommitStat> BuildCommitStats(Commit commit)
        {
            ...
            foreach (var parent in commit.Parents)
            {
                ...
                var commitStats = new CommitStat
                {
                    CommitDateTime = parent.Committer.When.DateTime
                };
                ...
                foreach (var file in fileChanges)
                {
                    ...
                    commitStats.Added += file.LinesAdded - totalCommentedOutlinesToRemove;
                    commitStats.Removed += file.LinesDeleted;
                }

                result.Add(commitStats);
            }
        ...
        private CommitStat BuildFirstCommitStats(Commit commit)
        {
            var stats = _repository.Diff.Compare<PatchStats>(null, commit.Tree);
            var result = new CommitStat
            {
